...
function profilePage() {
  const body = `
    ...
    <form id="profile-form">
      ...
      <div class="field">
        ...
        <input id="avatar-input" data-testid="avatar-upload" type="file" accept="image/*" />
      </div>
      <div class="row">
        <img data-testid="avatar-image" src="${state.user.avatarSrc}" width="120" height="120" alt="Profile avatar" />
        <button data-testid="remove-avatar-btn" type="button">Remove avatar</button>
      </div>
      <button data-testid="submit-btn" type="submit">Save changes</button>
  ...
  const script = `
    ...
    const avatar = document.querySelector('[data-testid="avatar-image"]');
    const removeAvatarButton = document.querySelector('[data-testid="remove-avatar-btn"]');
